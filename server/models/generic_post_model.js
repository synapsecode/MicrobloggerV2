module.exports = (sequelize, { PRIMARYKEY, INT, STRING, JSONTYPE }) => {
    ...
    class GenericPostModel {
        ...
        delete = async () => {
            ...
            //handle removal of reshare
            if (post_type.includes('reshare')) {
                await Associations.resharetrackertable.destroy({ where: { reshare_owner_id: this.genericpost_reference.id } });
            } else {
                if (post_type.includes('quote')) {
                    await Associations.quotetrackertable.destroy({ where: { quote_owner_id: this.genericpost_reference.id } });
                }

                let { reshares, quotes } = await this.get_reshares();
                //Delete all reshares associated with that post
                await Promise.all(reshares.map(x => x.delete()));
                //set target id of all its quotes to -1 which signifies a deleted post
                //TODO: Model-wide handling of -1 state
                let quote_owner_ids = quotes.map(x => x.genericpost_reference.id)
                await Promise.all(quote_owner_ids.map(x => Associations.quotetrackertable.update({quote_owner_id: -1,}, {
                    where: { quote_owner_id: x }
                })));

                //Destroy internal datatable
        ...
        reshare = async (user) => {
            //TODO: handle case when reshare target/quoted target has been deleted
